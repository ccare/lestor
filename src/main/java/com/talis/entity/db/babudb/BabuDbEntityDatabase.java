...
public class BabuDbEntityDatabase implements EntityDatabase{
	...
	static {
		BabuDBConfig config = new ConfigBuilder()
									...
									.build();
		CONFIG_TEMPLATE = config.getProps();
		CONFIG_TEMPLATE.setProperty("babudb.checkInterval", "30");
	}
...
//	private File dataDir;
	...
	public BabuDBConfig getConfiguration(File dbDir) throws IOException{
		Properties props = new Properties(CONFIG_TEMPLATE);
	...
	protected void initDatabase(){
        try{
    		...
    		BabuDBConfig conf = getConfiguration(dbDir);
    		System.out.println(conf.getCheckInterval());
    		System.out.println(conf.getDbLogDir());
        	dbSystem = BabuDBFactory.createBabuDB(getConfiguration(dbDir));
        	try{
